...
const camelCase = (input: string | string[], options: { pascalCase?: boolean } = {}) => {
...
export function toQueryString(
  ...
  return Object.keys(paramsObject)
    .filter(
      (key: string) =>
        paramsObject[key] !== '' && paramsObject[key] !== null && typeof paramsObject[key] !== 'undefined'
    )
    .map((key: string) =>
      Array.isArray(paramsObject[key])
        ? // convert to key=val1,val2,val3 string
          `${key}=${paramsObject[key].map((val: string | number) => `${encodeURIComponent(val)}`).join(',')}`
...
export function useRequest<T = any>(url: string, { lazy = true, variables = {}, ..._options }: UseRequestOptions = {}) {
  ...
  return {
    loading,
    mutate: _refetch as (v?: { [key: string]: any } | { [key: string]: any }[]) => Promise<T>,
    data,
    error,
    called
  };
...
const contentTypeIs = (s: string) => (response: Response) => response?.headers?.get('content-type')?.includes(s);

const convertObjectKeys: any = (object: any[] | { [key: string]: any }, keypath: string[] = []) => {
...
function request<T = any>(
  ...
  if (!isForm) {
    const formatted = format(_fullUrl, body);
  ...
  try {
    return fetch(_fullUrl, {
      ...
      .then(async (response: Response) => {
        // const isMutate = !!options.method && !!displayMessage && options.method !== 'GET';
        // if (isMutate) {
        //   message.success({
        ...
        if (response.status === 404) {
          console.log('error trong catch rlt', result);
          return Promise.reject(response.statusText);
        }
        if (response.status === 401) {
          Cookies.remove('token');
          // Xóa role lưu trong LC
      ...
      .catch((err: any) => {
        ...
        if (onError) {
          console.log('error trong catch 1', err);
    ...
    // bắt err tại đây luôn 401 (Unauthorized)
    console.log('error tại catch 2 đây', error);
